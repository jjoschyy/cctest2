...
class UpdateSalesorder extends Update {
    ...
    private $customer;
    private $salesorder;
    private $salesorderItemIds;
    private $salesorderPartnerIds;

    public function __construct($data) {
        parent::__construct($data, Salesorder::rules());
        $this->salesorderItemIds = array();
        $this->salesorderPartnerIds = array();
    }
    ...
    private function getLocation() {
        return \App\Location::unique()->where('plant', $this->data['Plant'])->firstOrFail();
    }

    private function updateSalesorderItems() {
        $this->salesorder->salesorderItems()->restore();
        foreach ($this->data['Items'] as $key => $data) {
            $item = $this->updateSalesorderItem($data);
            $this->salesorderItemIds[] = $item->id;
            $this->data['Items'][$key]['id'] = $item->id;
        }
        $this->salesorder->salesorderItems()->whereNotIn('id', $this->salesorderItemIds)->delete();
    }
    ...
    private function updateSalesorderItem($data) {
        ...
        $item->parent()->associate($this->getSalesorderItemParent($data));
        $item->save();
        return $item;
    }

    private function getSalesorderItemParent($data) {
        foreach ($this->data['Items'] as $value) {
            if ($value['ItemNumber'] == $data['HigherLevelItemNumber'])
                return new \App\SalesorderItem(['id' => $value['id']]);
        }
    }

    private function updateSalesorderPartners() {
        foreach ($this->data['Partners'] as $data) {
            $this->salesorderPartnerIds[] = $this->updateSalesorderPartner($data)->id;
        }
        $this->salesorder->salesorderPartners()->whereNotIn('id', $this->salesorderPartnerIds)->delete();
    }

    private function updateSalesorderPartner($data) {
        $partner = $this->salesorder->salesorderPartners()->firstOrNew(['customer_number' => $data['CustomerNumber']]);
